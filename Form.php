...
class Form extends Base {
	...
	public function __construct($id = "form", $action = null, $method = 'POST') {
		$this->configure(array(
			"action" => empty($action) ? basename($_SERVER["SCRIPT_NAME"]) : $action,
		...
		project are located.  This property will automatically be set properly if the Form directory
		is uploaded within the server's document root.  If symbolic links are used to reference the Form
		...
		} elseif (strpos(str_replace("\\", "/", $path), $_SERVER["DOCUMENT_ROOT"]) !== false) {
			$this->resourcesPath = substr($path, strlen($_SERVER["DOCUMENT_ROOT"]));
		} else {
			$this->resourcesPath = "/form/Resources";
		}
	}
	...
	public static function clearErrors($id = "form") {
		if (!empty($_SESSION["form"][$id]["errors"])) {
			unset($_SESSION["form"][$id]["errors"]);
		}
	}

	public static function clearValues($id = "form") {
		if (!empty($_SESSION["form"][$id]["values"])) {
			unset($_SESSION["form"][$id]["values"]);
		}
	}
	...
	public function getErrors() {
		$errors = array();
		if (session_id() == "") {
			$errors[""] = array("Error: The form project requires an active session to function properly.  Simply add session_start() to your script before any output has been sent to the browser.");
		} else {
			$errors = array();
			...
			if (!empty($_SESSION["form"][$id]["errors"])) {
				$errors = $_SESSION["form"][$id]["errors"];
			}
		}
	...
	protected static function getSessionValues($id = "form") {
		$values = array();

		if (!empty($_SESSION["form"][$id]["values"])) {
			$values = $_SESSION["form"][$id]["values"];
		}

		return $values;
	}

	public static function isValid($id = "form", $clearValues = true) {
		$valid = true;
	...
	protected static function recover($id) {
		if (!empty($_SESSION["form"][$id]["form"])) {
			return unserialize($_SESSION["form"][$id]["form"]);
		} else {
			return "";
		}
	}
	...
	form using json.*/
	public static function renderAjaxErrorResponse($id = "form") {
		$form = self::recover($id);
...
JS;
	...
	protected function save() {
		$_SESSION["form"][$this->_attributes["id"]]["form"] = serialize($this);
	}
	...
	protected static function _setSessionValue($id, $element, $value) {
		$_SESSION["form"][$id]["values"][$element] = $value;
	}
